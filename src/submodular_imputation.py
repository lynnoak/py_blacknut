...
class SubmodularWithNan (Submodular):
    ...
    def ChoMetricWithNan(self, X1, X2):
        ...
        """
        index1 = np.where(X1 == -1)
        index2 = np.where(X2 == -1)
        X = abs(X1 - X2)
        X[index1] = -1
        X[index2] = -1
        index = np.where(X ==-1)
        if len(index[0]) !=0:
            ...
            Xo[0] = 1 if Xo[0]==-1 else Xo[0]
            X[0] =Xo[0]
            X[1:] = np.array([Xo[i - 1] if Xo[i]==-1 else Xo[i] for i in range(1, len(Xo))])
    ...
    def ColKNNScore(self,n_neighbors = 5, scoring = ['acc']):
        ...
        S['Submodular with nan'] = CKS.ColKNNScore(self.X,self.y,title=title)
